...
union = lambda s, subset_idx = None: reduce(np.union1d, np.take(s, subset_idx if subset_idx else range(len(s)), axis=0))
union_non_sorted = lambda s: np.array(reduce(lambda a, b: a+b, s))
